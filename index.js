...
const pagingController = {
    ...
    toggle() {
        ...
        $(
            ".ih-folder-dropdown-portal [data-button-key='pagingMode']",
...
const autoScrollController = {
    ...
    _updateActiveUI(isActive) {
        ...
        $(
            ".ih-folder-dropdown-portal [data-button-key='autoScroll']",
        ).toggleClass("input-helper-btn-active", isActive);
    ...
    release() {
        ...
        if (this._chatEl) {
            ...
            if (this._touchHandler)
                this._chatEl.removeEventListener(
                    "touchmove",
                    this._touchHandler,
                );
        }
...
const findReplaceController = {
    ...
    _updateActiveUI(isActive) {
        ...
        $(
            ".ih-folder-dropdown-portal [data-button-key='findReplace']",
        ).toggleClass("input-helper-btn-active", isActive);
    ...
    _createBar() {
        ...
        bar.find('[data-action="replace"]').on("click", () =>
            this._doReplace(),
        );
        bar.find('[data-action="replaceAll"]').on("click", () =>
            this._doReplaceAll(),
        );
        bar.on("keydown", (e) => {
            if (e.key === "Escape") {
                ...
                e.key === "Enter" &&
                $(e.target).hasClass("ih-find-input")
            ) {
                e.preventDefault();
    ...
    _highlightMatch() {
        ...
        const desiredScroll =
            linesBefore * lineHeight - textarea.clientHeight / 3;
...
const historyManager = {
    ...
    pushState(textarea) {
        ...
        if (this.pointer >= 0 && this.states[this.pointer].text === text)
            return;
    ...
    updateButtons() {
        ...
        $("#input_undo_btn").toggleClass(
            "input-helper-btn-disabled",
            undoDisabled,
        );
        $("#input_redo_btn").toggleClass(
            "input-helper-btn-disabled",
            redoDisabled,
        );
...
function insertPair(left, right, cursorOffset) {
    ...
    const newText =
        text.substring(0, startPos) + insert + text.substring(endPos);
    ...
    _removeAutoHide() {
        ...
        if (sendTextarea) {
            if (this._ahTextareaFocus) {
                sendTextarea.removeEventListener(
                    "focus",
                    this._ahTextareaFocus,
                );
                sendTextarea.removeEventListener(
                    "click",
                    this._ahTextareaFocus,
        ...
        if (target && target.length) {
            ...
            if (!isNaN(elTop)) {
                if (elTop + elHeight > vvBottom - 10) {
                    ...
                    const newTop = Math.max(minTop, v.offsetTop + 10);
...
function syncToolbarButtonStyles(targetContainer) {
    ...
    if (!_cachedToolbarStyles || now - _cachedToolbarStylesTime > CACHE_TTL) {
        const referenceBtn = $(
            "#input_helper_toolbar .input-helper-btn:visible",
        )
            .not(".ih-folder-btn")
            .first();
        ...
        const hasCustomShadow =
            refStyle.boxShadow && refStyle.boxShadow !== "none";
        const hasBackdrop =
        ...
        _cachedToolbarStyles =
            Object.keys(styleMap).length > 0 ? styleMap : null;
        _cachedToolbarStylesTime = now;
    }
...
function renderFloatingPanelSettings() {
    ...
    container.on("input", "#ih_fp_ball_size", function () {
        ...
        if (ball && ball.length) {
            ...
            ball.find("i.fa-ellipsis").css(
                "font-size",
                Math.max(14, val / 3) + "px",
            );
        }
    });
    ...
    container.on("click", "#ih_fp_add_button", function () {
        ...
        pickerContent.find(".ih-picker-confirm-btn").on("click", function () {
            ...
            if (selected.length > 0) {
                ...
                selected.forEach((k) =>
                    getSettings().floatingPanel.buttons.push(k),
                );
                saveSettingsDebounced();
                renderFloatingPanelSettings();
                floatingPanelController.refresh();
...
function showButtonPicker(folderIndex) {
    ...
    content.on("click", ".ih-picker-item", function () {
        ...
        $(this).css(
            "background-color",
            !isSelected ? "rgba(100,149,237,0.2)" : "",
        );
    });
...
function setupShortcutInputs() {
    $(".shortcut-input")
        ...
        .on("keydown", function (e) {
            ...
            if (
                !["Control", "Alt", "Shift", "Meta", "Escape"].includes(e.key)
            ) {
            ...
            for (const key of Object.keys(getSettings().shortcuts)) {
                if (
                    key !== sk &&
                    getSettings().shortcuts[key] === shortcutString
                ) {
...
function handleGlobalShortcuts(e) {
    ...
    for (const key in shortcuts) {
        if (shortcuts[key] === shortcutString) {
            const isSendTextarea =
                document.activeElement === getMessageInput()[0];
        ...
        $(".ih-folder-children").each(function () {
            ...
            $(this).sortable({
                ...
                receive: function (_, ui) {
                    ...
                    const targetFi = parseInt(
                        $(this).attr("data-folder-index"),
                    );
                    const srcFi = ui.item.data("ih-source-folder");
                    if (srcFi !== null && srcFi !== undefined) {
                        const srcFolder = getSettings().folders[srcFi];
                        ...
                        if (orderIdx > -1)
                            getSettings().buttonOrder.splice(orderIdx, 1);
                    }
...
async function loadSettings() {
    ...
    if (s.autoScrollToAiOnStream === undefined)
        s.autoScrollToAiOnStream = false;
    if (s.lockScrollOnGeneration === undefined)
        s.lockScrollOnGeneration = false;
    if (s.twoRowMode === undefined) s.twoRowMode = false;
    if (s.twoRowOrder === undefined) s.twoRowOrder = "input-first";
    if (!s.floatingPanel)
        s.floatingPanel = { ...defaultSettings.floatingPanel };
    ...
    if (s.floatingPanel.buttonSize === undefined)
        s.floatingPanel.buttonSize = 12;
    if (s.floatingPanel.collapsed === undefined)
        s.floatingPanel.collapsed = true;
    if (s.floatingPanel.autoHide === undefined)
        s.floatingPanel.autoHide = false;
    ...
    $("#enable_auto_scroll_ai_stream").prop(
        "checked",
        s.autoScrollToAiOnStream,
    );
    $("#enable_lock_scroll_generation").prop(
        "checked",
        s.lockScrollOnGeneration,
    );
...
jQuery(async () => {
    ...
    if (!$("#enable_toolbar_pinned").length) {
        ...
        `);
        const dangerRow = $("#enable_confirm_dangerous").closest(
            ".ih-switch-row",
        );
    ...
    if (!$("#ih_two_row_order_row").length) {
        ...
        `);
        const twoRowToggle = $("#enable_two_row_mode").closest(
            ".ih-switch-row",
        );
    ...
    try {
        ...
        const themeObserver = new MutationObserver(() => {
            ...
            _themeChangeTimer = setTimeout(() => {
                ...
                updateToolbarMaxHeight();
                try {
                    const _sp = document.querySelector(
                        ".input-helper-settings",
                    );
